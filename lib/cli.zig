...
const CPU = @import("root").icpu.CPU;
...
pub fn appendCmdToHist(args: [][]const u8) !void {
    ...
    _ = try cmd_history.append(try line.toOwnedSlice());
    cmd_index = cmd_history.items.len;
}
...
pub fn getUniqueCmds() []CmdStruct {
    ...
    return unique_cmds.toOwnedSlice() catch {
        return &.{};
    };
}
...
pub fn promptWithArrows(alloc: std.mem.Allocator) ![][]const u8 {
    ...
    while (true) {
        ...
        switch (read_in) {
            ...
            '\n' => {
                var cmd_iterator = std.mem.tokenize(u8, try line.toOwnedSlice(), " ");
...
pub fn prompt(alloc: std.mem.Allocator) ![][]const u8 {
    ...
    return try cmdList.toOwnedSlice();
}
